...
> checkout before running. Derived JSONs (`controls.json`, the per-model catalogs, `*_alllayers.json`) are
> regenerated by the pipeline; they are not committed here.
...
Every headline number is tested against a null and is reproducible from `controls.json`:
