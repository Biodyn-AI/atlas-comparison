...
check("CKA n pairs", rf"CKA with a cell-shuffle null for \*\*all {N} model pairs\*\*",
...
check("two-model fold", rf"are enriched \*\*{N}-fold\*\* \(31 edges\)", two, "trrust2:curve.2.fold")
prec = CMC.get("2", {}).get("precision")
check("two-model precision %", rf"a precision of \*\*{N} %\*\*", None if prec is None else 100 * prec,
